...
The task consists of finding groups or clusters within a dataset consisting of 49 patients (observations) who provided responses to about 12 factors (variables) that motivate them. Each value is basically the mean value of about 7 questions people answered within the particular category/variable. 
...
This is to explore for missing values and to make sure that the data are on the same scale. This is because these values for the different variables are unlikely to be on the same scale.
...
The result shows the absence of missing values in the dataset.
...
The similarity metric I will use is the **distance**. For hierarchical clustering, I will compute the distance between the samples, using base R's $dist()$ function which calculates the Euclidean distance by default.
...
```{r complete_Hclust}
mypar() # impoved graphics
hc_complete <- hclust(dist_scaled_data, method = "complete")
hc_complete
...
The hierarchical clustering with complete linkage is consistent with the existence of two main clusters, one of which has two main sub-clusters. So I define three clusters, and I cut the tree at height value 8 and group all samples that are within that distance into groups below. To visualize this, I will draw a horizontal line at the height = 8.
...
Results show which cluster each patient belongs to. The most populated clusters are clusters 3 and 2 while cluster 1 is the least.
...
# plot all linkage methods in one plot
par(mfrow = c(1,3))
...
We can also cluster with the base R's $kmeans()$ function to perform k-means clustering. To use kmeans clustering, I will empirically estimate the optimal number of Ks (clusters) using two approaches: 
...
The plot highlights the presence of 3 clusters.

It should be noted that good clustering can also be achieved with k = 4, but I go for k = 3 since it appears to have less cluster overlaps.

## **Step 4: Analyses.**

### **i. Principal components analysis to identify influential features.**

I perform PCA to identify features that determine the positions of the patients on the principal components and also if there are more hidden structures within the data set.
...
Just like the hierarchical clustering, the PCA plot of the first two principal components shows that P1, P23, and P44 form a cluster separated from the rest. However at the PCA level, P34 appears to highly correlate with P1, P23 and P44, a feature which hierarchical clustering misses out.
...
I will only assess the loading scores of the first PC since it accounts for most of the variation in the data set. The loading scores show that the variables **Macht_und_Einfluss**,  **Eros_und_Schoenheit**, **Wettkampf** and **Essen** tend to have a strong influence in pushing samples to the right of the PC1 vs PC1 plot. This also means that these variables have a strong correlation with each other. On the other hand **Emotionale_Ruhe** tends to push samples to the left of the PC1 vs PC2 plot. This shows that the factors  **Macht_und_Einfluss**,  **Eros_und_Schoenheit**, **Wettkampf**, **Essen** and **Emotionale_Ruhe** are very strong determinants of motivation.

### **ii. Correlation analysis.**

I perform a Spearman correlation analysis, to confirm findings from the PCA plot to see how features correlate with each other.
...
The results confirm that **Macht_und_Einfluss** has its strongest correlation with **Eros_und_Schoenheit**, **Wettkampf**  and negatively correlates with **Emotionale_Rule**.
...
# plot
set.seed(123)
plot(rp, branch = 0, margin = 0.1); text(rp, digits = 3, use.n = TRUE); title("Recursive partitioning")
...
Rpart analysis shows that, members of cluster 3  are more motivated by **Macht_und_Einfluss** compared members of cluster 2 and that members of cluster 1 are more motivated by **Wettkampf ** compared to members of cluster 3. Some members of cluster 3 have are highly motivated by **Eros_und_Schoenheit** while other members of the same cluster 3 have a low motivation for it. Cluster 1 has a higher motivation for **Eros_und_Schoenheit** compared to cluster 2. **Eros_und_Schoenheit** is a very important differentiating factor in the patients. 

### **iv. Heatmap visualisation.**

```{r heatmap}
rv <- rowVars(t(scaled_data)) # select variable features
idx <- order(-rv) # order varies features in decreasing order
...
The heatmap shows that the clustering is mostly homogeneous and quite successful, except for one member of cluster 2 which clusters together with members of cluster 1. Two members of cluster 1 cluster together with cluster 3.

It is noticeable that members of cluster 2 are in general the least motivated given all variables.
